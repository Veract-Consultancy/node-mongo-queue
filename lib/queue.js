...
(function() {
  ...
  exports.Connection = (function(superClass) {
    ...
    Connection.prototype.ensureConnection = function(opt) {
      ...
      else if (opt.db instanceof mongodb.Db) {
        db = opt.db;
    ...
    Connection.prototype.reset = function(callback) {
      ...
      return this.exec(function(collection) {
        var now = new Date();
        var timeout = new Date(now.getTime() + 12 * 60 * 60 * 1000);

        return collection.updateMany({}, {$set: {
          expires: timeout,
          timeout: timeout,
          owner: null
        }}, function(err, data){
          console.log("Err setting queue:", err);
          callback();
        });
      });
    }
    ...
    Connection.prototype.next = function(queue, owner, callback) {
      ...
      return this.exec(function(collection) {
        return collection.findOneAndUpdate(query, update, options, function(
          err,
          result
        ) {
    ...
    Connection.prototype.complete = function(doc, callback) {
      return this.exec(function(collection) {
        ...
        return collection.findOneAndDelete(query, options, function(
          err,
          result
        ) {
          return callback(err, result != null ? result.value : void 0);
          ...
          return function(err, doc) {
            if (err != null && err.message !== "No matching object found") {
              ...
              if (Template) {
                ...
                _this.emit(
                  "error",
                  new Error("Unknown template '" + _this.name + "'")
                );
              }
